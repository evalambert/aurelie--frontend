...
const Atlas = ({ atlases, lang }) => {
  ...
  atlases.forEach(a => {
    if (a.territory) {
      ...
      const territoryObj = typeof a.territory === 'object' ? a.territory : { territory: a.territory, index: null };
      ...
      if (!territoriesMap.has(territoryName) || 
          (territoryObj.index !== null && 
           (territoriesMap.get(territoryName).index === null || 
            Number(territoryObj.index) < Number(territoriesMap.get(territoryName).index)))) {
    ...
    if (item.siblings?.length) {
      item.siblings.forEach(sib => {
        ...
        if (activeTerritory !== 'all') {
          const siblingTerritory = fullSibling.territory?.territory || fullSibling.territory;
      ...
      <div className="wrapper-atlas pt-[53px] lg:pt-[40px] py-y-body px-x-body bg-[#E0E0E0] min-h-screen">
        ...
        <div className=' sticky top-[60px] lg:top-[40px]'>
          ...
          <ul className="filters flex flex-wrap gap-x-[10px] gap-y-[3px] lg:top-[40px] mb-[50px]">
            {lang === 'fr' ? 'territoires :' : 'territories :'} 
            {territoriesList.map((t) => (
              <FilterAtlasList
                key={t}
                mediumName={t}
                onClick={() => filterTerritory(t)}
                isActive={activeTerritory === t}
              />
            ))}
          </ul>
        </div>
