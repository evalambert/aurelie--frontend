...
const SliderLandscape = ({ slider, mode, onLeave }) => {
    ...
    const opacityClass = {
        current: "opacity-30",
        next: "opacity-[5%] pointer-events-none",
    ...
    return (
        <>
            ...
            <div className={`${loaded ? 'opacity-100' : 'opacity-0 pointer-events-none'} slider-landscape transition-opacity duration-700`}>
                <div className={`fixed top-y-body md:left-[41.2vw] lg:left-[42vw] md:w-[57.8vw] lg:w-[58vw] z-20 hover:[&+div]:opacity-100 h-[calc(100vh-(var(--spacing-y-body)*2))] ${opacityClass}  ${backgroundOn ? 'z-100 !opacity-100 grayscale-0' : 'grayscale-100'}`} onMouseEnter={() => setBackgroundOn(true)} onMouseLeave={() => { setBackgroundOn(false); if (onLeave) onLeave(); }}>
                    <img src={imageCoverUrl} className={`pointer-events-none max-h-[calc(100vh-(var(--spacing-y-body)*2))] ${isCoverLandscape ? 'md:max-w-[calc(57.8vw-var(--spacing-x-body))] lg:max-w-[calc(58vw-var(--spacing-x-body))]' : 'pl-[10px]'}`} loading="lazy" onLoad={() => setLoaded(true)} onError={() => setLoaded(true)} ref={(img) => { if (img?.complete) setLoaded(true); }} alt="" />
                </div>
